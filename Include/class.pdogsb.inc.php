...
class PdoGsb {
    ...
    private static $bdd = 'Database=GSB_VALIDE_KREHACA';
    private static $user = 'afc_krehaca';
    private static $mdp = 'afc_krehaca';
    ...
    public function getInfosComptable($login, $mdp) {
        $requete = PdoGsb::$monPdo->prepare('exec identificationComptable :login, :mdp');
    ...
    public function dernierMoisSaisi($idVisiteur) {
        $req = "select max(FICHE_MOIS) as dernierMois from FICHE_FRAIS FF where FF.VIS_ID = '$idVisiteur'";
        $res = PdoGsb::$monPdo->query($req);
        $laLigne = $res->fetch();
    ...
    public function getLigneFraisForfaitise ($numVisiteur, $mois) {
        
    }
    
    public function etatFicheFrais ($numVisiteur, $mois) {
        $requete = PdoGsb::$monPdo->prepare('exec etatFiche :mois, :visiteur');
        $requete->bindParam(':visiteur', $numVisiteur);
    ...
    public function infosFraisForfait($mois, $visiteur) {
        $requete = PdoGsb::$monPdo->prepare('exec infosFraisAuForfait :mois, :visiteur');
        $requete->bindParam(':visiteur', $numVisiteur);
        $requete->bindParam(':mois', $mois);
        $requete->execute();
    ...
    public function listeVisiteurs() {
        $requete = PdoGsb::$monPdo->prepare('exec listeVisiteurs');
        $requete->execute();

        return $requete;
    }

    public function infosFicheFrais($mois) {
        $requete = PdoGsb::$monPdo->prepare('exec infosFicheFrais :mois');
        $requete->bindParam(':mois', $mois);
        $requete->execute();
    ...
    function getInfosFiche() {
        $requete = PdoGsb::$monPdo->prepare('exec getInfosFF');
        $requete->execute();
        
        return $requete;
    }

    function getLignesFF($idVisiteur, $mois) {
        $requete = PdoGsb::$monPdo->prepare('exec getLignesFicheFrais :visiteur, :ficheMois');
        $requete->bindParam(':visiteur', $idVisiteur);
        $requete->bindParam(':ficheMois', $mois);
        $requete->execute();
        
        return $requete;
    }

    function getLignesFHF($idVisiteur, $mois) {
        $requete = PdoGsb::$monPdo->prepare('exec getLignesFraisHorsForfait :visiteur, :ficheMois');
        $requete->bindParam(':visiteur', $idVisiteur);
        $requete->bindParam(':ficheMois', $mois);
        $requete->execute();
